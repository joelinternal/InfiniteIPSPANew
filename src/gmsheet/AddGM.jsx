...
const AddGM = ({ refreshlst, setRefreshlst, screen, accountdata, appAccountId, appProjectId, appSowId }) => {
    ...
    const [canNewGMSheetAdded, setCanNewGMSheetAdded] = useState(true);
    const [showErrNewGMSheetAdded, setShowErrNewGMSheetAdded] = useState("");
    ...
    const handleSubmitGMSheet = () => {
        axios.post(`http://localhost:5071/api/GM/${accountId}/${projectId}/${sowId}`)
            .then(res => {
    ...
    const getdata = (accountId, projectId, isErrorShown = true) => {
        axios.get(`http://localhost:5071/api/GM/${accountId}/${projectId}/${sowId}/0`).then(res => {
            setRefreshGmSheetCalc(a => !a);
            if (res.data?.listGmSheet?.length == 0) {
                setlistdata([])
                setCanNewGMSheetAdded(res.data?.canAddNew);
                setShowErrNewGMSheetAdded(isErrorShown ? "Already Project Exist! Please use Open/Edit Run Sheet Window to Add New!" : "")
            }
            else if (res.data?.listGmSheet?.length > 0) {
                setlistdata(res.data.listGmSheet)
            } else {
                setlistdata([])
            }
        })
